...
class Xrm10VoiceAssistantLayout(NavScroller):
  def __init__(self):
    ...
    self._mode = BigMultiParamToggle("assistant mode", "Xrm10VoiceAssistantMode", ["off", "push to talk", "text bridge"])
    ...
    self._scroller.add_widgets([
      Xrm10ColorCard("voice assistant", "chat-style helper through a phone/server bridge", "cyan"),
      self._status,
      self._bridge,
      self._enabled,
      self._mode,
  ...
  def _request_listening(self):
    ...
    gui_app.push_widget(BigDialog("", "Listening requested.\n\nYour phone/server bridge must handle speech, model response, and audio playback."))
